...
runners.map (function (toUpper, index) {
  allCaps[index] = toUpper.first_name.toUpperCase();
});
...
let ticketPriceTotal = runners.reduce((total, donAmt) => {
  return total += donAmt.donation;
},0)
...
// Problem 1: List of companies involved with the race and how much they donated
// The winner of the race wins a purse 20% of the ticket sales. How much do we owe the winner this year?

let eventPurse = Math.round(ticketPriceTotal * .2);

console.log(eventPurse)

// Problem 2
//The event director wants to make a mailing list to send out a mass email to all of the runners.
let emails=[];
...
console.log(emails);
// Problem 3
// The event director wants to know what the top donation amount was

let maxDonations = runners.reduce ((max, amt) =>
  (amt.donation > max ? amt.donation : max),
  runners[0].id
);
